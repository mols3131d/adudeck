...
dbt는 작성한 모델과 메타데이터(`schema.yml`)를 기반으로 프로젝트 전체의 의존성 구조(Lineage)와 컬럼 설명을 웹
브라우저에서 열람 가능한 문서로 자동 생성해 줍니다.
...
`schema.yml`에 모델명이나 컬럼명을 잘못 입력하면, 빌드는 통과하더라도 문서에 해당 설명이 누락되거나 오류가 발생할 수
있습니다. 파일명과 모델명이 일치하는지 항상 주의하세요.
