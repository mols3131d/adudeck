...
데이터 웨어하우스(DuckDB) 환경에서 dbt의 핵심 역할(Transform, Test, Document)을 직접 구성해 보며, 파편화된 원천 데이터를 분석 가능한 마트 데이터로 탈바꿈시키는 전체 파이프라인을 구축합니다.

## dbt의 핵심 개념

dbt는 데이터를 가져오는(Extract/Load) 도구가 아니라, 이미 데이터베이스에 적재된 데이터를 변환(Transform), 검증(Test), 문서화(Document)하는 도구입니다. dbt를 구성하는 핵심 개념은 다음과 같습니다:
...
dbt는 데이터를 가져오는 도구가 아니라, 이미 데이터 웨어하우스나 로컬 분석 DB에 있는 데이터를 SQL로 변환하고 검증하고 문서화하는 도구입니다.
...
기존 데이터 웨어하우스나 데이터베이스에 데이터가 적재되어 있으면, dbt를 활용해 이를 정제(Staging), 관계 형성(Intermediate), 최종 마트(Marts) 테이블로 유연하게 모델링할 수 있습니다.
