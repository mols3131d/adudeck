...
여기서 `client.responses.create(**call_args)`의 `**`는 OpenAI SDK 문법이 아니라 **Python mapping unpacking**이다.
dictionary의 key를 keyword argument 이름으로, value를 그 argument의 값으로 펼친다. 현재 세 field에 대해서는 다음 호출과
같은 의미다.
