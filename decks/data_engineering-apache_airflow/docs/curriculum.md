...
1. **Learning-environment calibration** — fresh/disposable state에서 preflight → `db migrate` → local Dag discovery →
   local test → standalone → cross-view observation을 실제로 실행해 documentation-backed expectation과 learner-visible
   runtime evidence가 일치하는지 확인한다.
