...
개념적으로 scheduler가 **무엇을 실행할지** 결정한다면 executor는 **그 실행을 어떤 execution backend에 보낼지** 담당한다고 볼 수 있다.
...
scheduler가 `extract_orders` TaskInstance를 runnable하다고 판단하면 executor로 보낸다.
...
scheduler의 핵심 책임은 실행 가능성을 판단하고 executor에 제출하는 것이다. 실제 user task code 실행은 worker execution context에서 일어난다.
