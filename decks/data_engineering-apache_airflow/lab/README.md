...
환경 변수로 제공하는 demo Connection은 task runtime에서 resolve되며 Airflow UI나 `connections list`에는 표시되지 않는다. 이 차이 자체를 credential/configuration storage와 task runtime lookup의 boundary로 해석한다.
