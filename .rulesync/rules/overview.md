...
- `rulesync.local.jsonc` must remain untracked. If it defines `targets`, it replaces the shared target set, so include the
  shared targets alongside any local additions.
